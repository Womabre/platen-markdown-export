...
Revisions:
    - {Revision: 1, Date: "2026-04-08", Author: platen-markdown-export, Remarks: Initial showcase document}
...
# Named styles:
#   none | Navy | Grey | Blue | Green | Amber | Red
# Color accepts a palette name from this theme (or any hex value):
#   Navy | Grey | Blue | Green | Amber | Red
Style:
    Color: Navy
    Image: https://images.unsplash.com/photo-1511884642898-4c92249e20b6
    # Photo by <a href="https://unsplash.com/@pinewatt?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">pine  watt</a> on <a href="https://unsplash.com/photos/aerial-shot-of-forest-2Hzmz15wGik?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
Header: platen-markdown-export Feature Showcase  # blank = use Title
...
Every document starts with a **YAML frontmatter block** delimited by `---`. It controls the cover page, revision table, branding, and export behaviour.
...
Revisions:
    - {Revision: 1, Date: "2026-04-08", Author: platen-markdown-export, Remarks: Initial release}
Lang: en                       # <html lang>; default en

# ── Branding ──────────────────────────────────────────────────────
Theme: Default                 # brand package — see --list-themes
Style: Navy                    # or Grey, Blue, Green, …
...
**Title & subtitle.** On the cover, the `Title` is split at the first *spaced* em-dash (`A — B`), en-dash, or colon (`A: B`): the part before becomes the headline and the part after a smaller subtitle. This document's title renders **platen-markdown-export** as the headline and *A Feature Showcase* as the subtitle. A title without a separator is shown as a single headline — no empty subtitle appears.
...
| Key | Description |
| --- | ----------- |
| `Theme` | Brand package — `Default` here; see `--list-themes` |
| `Style` | Cover variant: a named style (`Navy`, `Grey`, `Blue`, …), or `{Color, Image, Position}` |
...
Inline `code` is rendered with a light grey background — use it for keys like `Revisions Visible`, commands like `npm run build`, or values like `210mm`.
...
Links look like this: [Markdown spec](https://commonmark.org). They are clickable in the PDF.

Emoji are supported too 🎉 — from classics like ✅ 🚀 ⚠️ to skin-tone variants 👋🏻 👋🏽 👋🏿 and recent additions: pink heart 🩷, jellyfish 🪼, phoenix 🐦‍🔥 and paint splatter 🫟.
...
Tables are rendered with the active theme's style. Columns can be left-, center-, or right-aligned using colons in the separator row.

| Feature      | Engine             | Notes                     |
| ------------ | ------------------ | ------------------------- |
| **PDF**      | WeasyPrint         | Print-quality, paged      |
| **Diagrams** | Mermaid + Chromium | Rendered to SVG, embedded |
| **Math**     | KaTeX              | Inline and display blocks |
...
Fenced code blocks use `highlight.js` for syntax highlighting. Specify the language after the opening triple-backtick.

**Shell:**

```sh
npm run build
node dist/index.js --mode pdf,html --theme default "document.md"
```
...
function outputName(input: string, revision: number): string {
    return input.replace(/\.md$/, `_Rev${revision}.pdf`);
}
```
...
Mermaid diagrams are rendered to SVG by Playwright/Chromium and embedded directly in the PDF.

**The export pipeline:**

```mermaid
flowchart TD
    MD([Markdown]) --> Parse[markdown-it + plugins]
    Parse --> Pre[Mermaid & image preprocessing]
    Pre --> PDF[Cover · headers · TOC → WeasyPrint]
    Pre --> HTML[Banner · dark mode → standalone HTML]
    PDF --> Out([_Rev1.pdf])
    HTML --> OutH([_Rev1.html])

    style MD fill:#dbe7f3
    style Out fill:#c8e6c9
    style OutH fill:#c8e6c9
```
...
sequenceDiagram
    ...
    participant CLI as platen-markdown-export
    participant W as WeasyPrint
    U->>CLI: export document.md
    CLI->>CLI: build cover + TOC
    CLI->>W: serve HTML over localhost
    W-->>CLI: PDF bytes
    CLI-->>U: document_Rev1.pdf
```
...
` ```dot ` (or ` ```graphviz `) fences are laid out to SVG by a WASM build of Graphviz — no browser, no network, no external binary.

```dot
digraph {
    rankdir=LR;
    node [shape=box, style=rounded];
    Markdown -> Parse -> Render -> PDF;
    Render -> Error [label="on failure", color=red];
}
```
...
` ```infographic ` fences render [AntV Infographic](https://github.com/antvis/infographic)'s declarative syntax — lists, sequences, hierarchies, comparisons and charts, 270-odd templates — with no browser. Start the block with `infographic <template-name>`; icons are [Iconify](https://icon-sets.iconify.design/) names such as `mdi/pencil-outline`. Many templates give text a fixed slot, so keep labels short.
...
data
  lists
    - label Write
      desc Markdown and frontmatter
      icon mdi/pencil-outline
    - label Render
      desc Diagrams, math, images
      icon mdi/cog-outline
    - label Publish
      desc A branded PDF and HTML
      icon mdi/file-pdf-box
```

Figure: From Markdown to a branded PDF
...
> [!NOTE]
> Icons come from Iconify by default, and only their names are sent. `--infographic-icons none` makes no request at all; `--infographic-icons weavefox` uses Ant Group's WeaveFox service instead — **data may go to servers in China**.

---

## Math — KaTeX {.page-break-before}

Mathematical expressions are rendered by KaTeX — both inline and as display blocks.

The A-series keeps a constant aspect ratio (inline): a sheet of ratio $1:\sqrt{2}$ halves into two sheets of the same ratio.

**Display block** — an A-series sheet area:

$$
A_n = \frac{1}{2^{\,n}}\ \text{m}^2 \qquad (A0 = 1\,\text{m}^2)
$$

So A4, five halvings down from A0, has area:

$$
A_4 = \frac{1}{2^{4}} = \frac{1}{16}\,\text{m}^2 \approx 0.0625\,\text{m}^2
$$
...
GitHub-style alerts use the `> [!TYPE]` blockquote prefix:

> [!NOTE]
> The `default` theme is the neutral fallback, used whenever no `Theme` is set.

> [!TIP]
> Use `--mode pdf,html` to produce both a PDF and a standalone HTML file in one run.

> [!IMPORTANT]
> The cover page only appears when a `Style` is present.

> [!WARNING]
> Editing files in `dist/` is pointless — they are regenerated from `src/` on every build.

> [!CAUTION]
> `--no-bump` is required in CI, or exporting a `Released` document mutates the source file.

### MkDocs Admonitions

MkDocs-style admonitions use `!!! type` with indented content and an optional title:

!!! note
    Default note admonition — no custom title.

!!! tip Pro tip
    Add a title after the type keyword to label any admonition.

!!! warning External runtime required
    WeasyPrint is not an npm dependency — install it separately (`brew install weasyprint`).

!!! success
    All feature checks passed. The document is ready to export.
...
> Content is written once, in Markdown; the theme decides how it looks.
>
> — The design goal of platen-markdown-export

**Footnotes:**

Diagrams render through headless Chromium.[^mermaid] The PDF engine is a separate binary.[^weasy]

[^mermaid]: Playwright Chromium, used only when a `mermaid` block is present.
[^weasy]: WeasyPrint — discovered at known install paths, not via `PATH`.
...
Defined terms are highlighted on hover and collected into a **Glossary** appendix. For example: Markdown is converted to HTML, styled with CSS, and printed to PDF with a rebuilt TOC.
...
Two icon libraries are bundled automatically when their tags appear in the document.

### Font Awesome

Inline SVG icons from [Font Awesome Free](https://fontawesome.com/). Use `<i class="fa-solid|regular|brands fa-icon-name">`.

| Style    | HTML                                                          | Result                                                          |
| -------- | ------------------------------------------------------------- | --------------------------------------------------------------- |
| Solid    | `<i class="fa-solid fa-file-pdf">`                            | <i class="fa-solid fa-file-pdf"></i>                            |
| Brand    | `<i class="fa-brands fa-markdown">`                           | <i class="fa-brands fa-markdown"></i>                           |
| Coloured | `<i class="fa-solid fa-circle-check" style="color:#22863a;">` | <i class="fa-solid fa-circle-check" style="color:#22863a;"></i> |

### Phosphor Icons

[Phosphor Icons](https://phosphoricons.com/) offer six weights, set by the base class (`ph`, `ph-bold`, `ph-fill`, …):

<i class="ph ph-file-text"></i> regular &nbsp; <i class="ph-bold ph-file-text"></i> bold &nbsp; <i class="ph-fill ph-check-circle" style="color:#22863a;"></i> fill

---

## Page Breaks & Layout

Add `{.page-break-before}` after a heading to start that section on a new page:

```markdown
## My Section {.page-break-before}
```

The tool strips the attribute from both the TOC link and the anchor. Add `<!-- omit in toc -->` after a heading to keep it out of the Table of Contents — as done on the Contents heading above.

---

## Theme & Branding {.page-break-before}

A **theme** is a swappable brand package (logos, fonts, CSS, cover templates, colour palettes and styles) living in a folder under `themes/`. Select one with the `Theme` frontmatter key (or `--theme`). A **style** picks the per-document cover variant. **This document is rendered with the `default` theme** — the brand-neutral fallback, aligned with the [Marp default theme](https://github.com/marp-team/marp-core).
...
The `default` theme defines 6 cover styles, each mapping to a colour palette whose **main** colour drives the cover gradient and tints headings and table headers:
...
Every palette ramps from light to dark — `extraLight` → `light` → `main` → `dark` → `extraDark`:

**Navy** — `#dbe7f3` · `#4488cc` · `#224466` · `#16304a` · `#0d1d2e`

**Grey** — `#f4f5f7` · `#e4e7eb` · `#1f2933` · `#3e4c59` · `#111827`
...
```html
<div class="default-navy-scheme">Navy — heavy scheme</div>
<div class="default-navy-light-scheme">Navy — light scheme</div>
```

<div class="default-navy-scheme">Navy — heavy scheme</div>
<div class="default-navy-light-scheme">Navy — light scheme</div>
<div class="default-grey-scheme">Grey — heavy scheme</div>
<div class="default-grey-light-scheme">Grey — light scheme</div>

Available in this theme: `default-navy-scheme` · `default-grey-scheme` · `default-blue-scheme` · `default-green-scheme` · `default-amber-scheme` · `default-red-scheme` (each with a `-light-scheme` variant).
...
Instead of a named style, provide a `Color` (required) and an optional `Image` URL for the hero background:
...
The **Default** theme sets both body and heading typography to:
