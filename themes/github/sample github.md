---
Title: GitHub Flavored Markdown — Writing Docs on GitHub  # the part after " — ", " – " or ": " becomes the cover subtitle
Document Info:
    Author: The Octocat
    Date: "2026-04-08"
    Revision: 1
...
Revisions:
    - {Revision: 1, Date: "2026-04-08", Author: The Octocat, Remarks: Initial showcase document}
...
Style:
    Color: GitHub
    Image: https://images.unsplash.com/vector-1751489957671-b9321ed106d6
    # <a href="https://unsplash.com/illustrations/coding-windows-with-lines-of-text-are-shown-U7qNLqf_Xcs?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Illustration</a> by <a href="https://unsplash.com/@publicdomainvectors/illustrations?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Public domain vectors</a> on <a href="https://unsplash.com/illustrations?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
Header: GitHub Feature Showcase             # blank = use Title
...
# GitHub Flavored Markdown — Writing Docs on GitHub

## Contents<!-- omit in toc -->

- [GitHub Flavored Markdown — Writing Docs on GitHub](#github-flavored-markdown--writing-docs-on-github)
...
**GitHub Flavored Markdown** (GFM) is the CommonMark superset used across GitHub — READMEs, issues, pull requests and wikis. It adds tables, task lists, strikethrough, autolinks and alerts. The **`github`** theme mirrors github-markdown-css.

```yaml
---
# ── Document identity ─────────────────────────────────────────────
Title: GitHub Flavored Markdown — Writing Docs on GitHub
...
Revisions:
    - {Revision: 1, Date: "2026-04-08", Author: The Octocat, Remarks: Initial release}
Lang: en                       # <html lang>; default en

# ── Branding ──────────────────────────────────────────────────────
Theme: GitHub                  # brand package — see --list-themes
Style: GitHub                  # a named style
...
**Title & subtitle.** On the cover the `Title` is split at the first *spaced* em-dash (`A — B`), en-dash, or colon (`A: B`). Here **GitHub Flavored Markdown** is the headline and *Writing Docs on GitHub* the subtitle. A title without a separator renders as a single headline.
...
| Key | Description |
| --- | ----------- |
| `Theme` | Brand package — `GitHub` here; see `--list-themes` |
| `Style` | Cover variant: a named style (`GitHub`), or `{Color, Image, Position}` |
...
Inline `code` suits things like `git rebase -i` or `.github/workflows/ci.yml`.
...
Autolinks just work: https://github.com. Named links look like this: [GitHub Docs](https://docs.github.com).

Emoji are supported too 🐙 — shortcodes like `:rocket:` render on GitHub; here we use Unicode 🚀 ✅ ⚠️, plus pink heart 🩷 and jellyfish 🪼.
...
Tables are a GFM extension. Columns can be left-, center-, or right-aligned.

| Element       | Syntax       | Renders as   |
| ------------- | ------------ | ------------ |
| **Mention**   | `@octocat`   | a user link  |
| **Issue ref** | `#42`        | a cross-link |
| **Task**      | `- [ ] todo` | a checkbox   |
...
Fenced code blocks use `highlight.js` for syntax highlighting — GitHub uses `linguist`.

**Shell — a git workflow:**

```sh
git switch -c feature/cover-subtitle
git add -A && git commit -m "Add title/subtitle split"
git push -u origin HEAD
gh pr create --fill
```

**YAML — a GitHub Actions workflow:**
...
GitHub renders Mermaid in Markdown; so does this tool — to SVG, embedded in the PDF.
...
flowchart LR
    ...
    style Fork fill:#ddf4ff
    style Merge fill:#dafbe1
    style CI fill:#fff8c5
```
...
GitHub supports `$…$` and `$$…$$` math; here it is rendered by KaTeX.

The probability that two random commits share a short-SHA prefix of $n$ hex digits (inline) is roughly $16^{-n}$.

**Display block — birthday bound for a 7-char short SHA:**

$$
P(\text{collision}) \approx 1 - e^{-\frac{k(k-1)}{2 \cdot 16^{7}}}
$$

For $k = 10{,}000$ objects:

$$
P \approx 1 - e^{-\frac{10^{8}}{5.4\times 10^{8}}} \approx 0.17
$$
...
Two syntaxes are supported: **GitHub Alerts** — GitHub's own feature — and **MkDocs-style admonitions**.

### GitHub Alerts

> [!NOTE]
> Alerts render natively on GitHub using this exact `> [!TYPE]` syntax.

> [!TIP]
> Reference issues and PRs with `#number`; they become cross-links automatically.

> [!IMPORTANT]
> Protected branches can require green status checks before a merge.

> [!WARNING]
> Force-pushing a shared branch rewrites history for everyone tracking it.

> [!CAUTION]
> Never commit secrets — rotate any credential that lands in git history.
...
!!! tip Draft PRs
    Open a draft pull request to run CI early without requesting review.

!!! abstract Summary
    GFM = CommonMark + tables + task lists + strikethrough + autolinks + alerts.

!!! success
    All checks have passed. This branch is mergeable.
...
> Real developers ship. And they ship with a green build.
>
> — Continuous Integration folklore

**Footnotes:**

GFM is specified as a formal superset of CommonMark.[^gfm] Alerts are a more recent addition.[^alerts]

[^gfm]: GitHub Flavored Markdown Spec, based on CommonMark.
[^alerts]: GitHub Alerts, generally available since 2023.
...
For example: a PR triggers CI, which is keyed by the commit SHA; GFM renders the whole thread.
...
| Style    | HTML                                                          | Result                                                          |
| -------- | ------------------------------------------------------------- | --------------------------------------------------------------- |
| Brand    | `<i class="fa-brands fa-github">`                             | <i class="fa-brands fa-github"></i>                             |
| Solid    | `<i class="fa-solid fa-code-branch">`                         | <i class="fa-solid fa-code-branch"></i>                         |
| Coloured | `<i class="fa-solid fa-circle-check" style="color:#1a7f37;">` | <i class="fa-solid fa-circle-check" style="color:#1a7f37;"></i> |

### Phosphor Icons

<i class="ph ph-git-branch"></i> regular &nbsp; <i class="ph-bold ph-git-pull-request"></i> bold &nbsp; <i class="ph-fill ph-check-circle" style="color:#1a7f37;"></i> fill
...
A **theme** is a swappable brand package under `themes/`. The **`github`** theme reproduces the familiar GitHub Markdown look — its colours and typography come from github-markdown-css.
...
The `github` theme ships a single style whose **main** colour drives the cover accents and tints headings and table headers:
...
The **GitHub** ramp — `extraLight` → `light` → `main` → `dark` → `extraDark`:

**GitHub** — `#f6f8fa` · `#7c7f82` · `#24292e` · `#191d20` · `#0d1117`
...
```html
<div class="github-github-scheme">GitHub — heavy scheme</div>
<div class="github-github-light-scheme">GitHub — light scheme</div>
```

<div class="github-github-scheme">GitHub — heavy scheme</div>
<div class="github-github-light-scheme">GitHub — light scheme</div>

Available in this theme: `github-github-scheme` (each with a `-light-scheme` variant).
...
The **GitHub** theme uses the system UI sans-serif stack GitHub itself ships:

`-apple-system, "Segoe UI", "Noto Sans", Helvetica, Arial, sans-serif`
